...
### D1 (low, дельта-ревью) — split-ветка `title_of` была не покрыта

`test_skip_token_does_not_leak_into_the_printed_title` проверял только пункты с одним
разделителем, где название — весь хвост строки. Ветка с двумя разделителями
(`rest.split(end, 1)[0]` — формат самого `DAY0.md`) выживала после снятия очистки.
В тест добавлен третий пункт с двумя разделителями и токеном в первом поле, и точная
проверка напечатанной строки, а не только отсутствия `[skip:`.

| Мутация | Упавший тест |
|---|---|
| в split-ветке `SKIP_TOKEN.sub("", rest.split(end, 1)[0])` → `rest.split(end, 1)[0]` | `test_skip_token_does_not_leak_into_the_printed_title` (1 failed, 14 passed) |

Отличие от текста ревью: код пункта взят `R5.2`, а не `R6.5` — `R6.5` уже занят первым
пунктом этой же фикстуры, и дубль кода сделал бы проверку напечатанной строки
двусмысленной. Проверяемое поведение то же.
